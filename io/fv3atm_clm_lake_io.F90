...
!> fv3atm_restart_io.F90 should ever access these routines.
!>
...
module fv3atm_clm_lake_io
  ...
  type clm_lake_data_type
     ...
     real(kind_phys), pointer, private, dimension(:,:) :: T_snow=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: T_ice=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: lake_snl2d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: lake_h2osno2d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: lake_tsfc=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: clm_lakedepth=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: lake_savedtke12d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: lake_sndpth2d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: clm_lake_initialized=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:) :: input_lakedepth=>null() !> ???

     ! All 3D variables needed for a restart
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_snow_z3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_snow_dz3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_snow_zi3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_h2osoi_vol3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_h2osoi_liq3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_h2osoi_ice3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_t_soisno3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_t_lake3d=>null() !> ???
     real(kind_phys), pointer, private, dimension(:,:,:) :: lake_icefrac3d=>null() !> ???

     ! Axis indices in 1-based array, containing non-1-based indices
     real(kind_phys), pointer, private, dimension(:) :: levlake_clm_lake !> ???
     real(kind_phys), pointer, private, dimension(:) :: levsnowsoil_clm_lake !> ???
     real(kind_phys), pointer, private, dimension(:) :: levsnowsoil1_clm_lake !> ???
  contains
    ...
    !> Fill internal arrays with zero.
...
CONTAINS
  ...
  subroutine clm_lake_allocate_data(clm_lake,Model)
    ...
    allocate(clm_lake%input_lakedepth(nx,ny))
  ...
  subroutine clm_lake_fill_data(clm_lake, Model, Atm_block, Sfcprop)
    ...
    do nb = 1, Atm_block%nblks
      do ix = 1, Atm_block%blksz(nb)
        ...
        clm_lake%input_lakedepth(i,j) = zero
  ...
  subroutine clm_lake_register_fields(clm_lake, Sfc_restart)
    ...
    call register_restart_field(Sfc_restart, 'clm_lake_initialized', clm_lake%clm_lake_initialized, &
         dimensions=(/'xaxis_1', 'yaxis_1', 'Time   '/), chunksizes=chunksizes2d, is_optional=.true.)
    call register_restart_field(Sfc_restart, 'input_lakedepth', clm_lake%input_lakedepth, &
         dimensions=(/'xaxis_1', 'yaxis_1', 'Time   '/), chunksizes=chunksizes2d, is_optional=.true.)
  ...
  subroutine clm_lake_bundle_fields(clm_lake, bundle, grid, Model, outputfile)
    ...
    call create_2d_field_and_add_to_bundle(clm_lake%input_lakedepth, "input_lakedepth", trim(outputfile), grid, bundle)
  ...
  subroutine clm_lake_deallocate_data(clm_lake)
    ...
    IF_ASSOC_DEALLOC_NULL(input_lakedepth)
