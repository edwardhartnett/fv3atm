...
module fv3atm_rrfs_sd_io
  ...
  type rrfs_sd_state_type
    ...
    ! i,j variables
    !> ???     
    real(kind_phys), pointer, private, dimension(:,:) :: emdust=>null() 
    !> ???     
    real(kind_phys), pointer, private, dimension(:,:) :: emseas=>null()
    !> ???     
    real(kind_phys), pointer, private, dimension(:,:) :: emanoc=>null()
    !> ???     
    real(kind_phys), pointer, private, dimension(:,:) :: fhist=>null()
    !> ???     
    real(kind_phys), pointer, private, dimension(:,:) :: coef_bb_dc=>null()

    !>  i, j, fire_aux_data_levels.
    real(kind_phys), pointer, private, dimension(:,:,:) :: fire_in=>null() 
...
#undef IF_ASSOC_DEALLOC_NULL
  ...
  !> Called after register_emi() to copy data from internal arrays to
